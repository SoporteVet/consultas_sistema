...
let labTicketsActiveListener = null;
let labTicketsAllDataLoaded = false; // Flag para saber si ya se cargaron todos los datos
...
// Configurar listeners de Firebase para laboratorio (OPTIMIZADO - carga diferida)
function setupLabFirebaseListeners(loadAllData = false) {
    ...
    if (labTicketsAllDataLoaded && !loadAllData) {
        setupLabIncrementalListeners();
        return;
    }
    ...
    if (!loadAllData) {
        ...
        daysAgo.setDate(today.getDate() - labTicketsLoadDateRange);
    ...
    const loadQuery = loadAllData 
        ...
        : labTicketsRef.orderByChild('fechaCreacion').startAt(cutoffDate).once('value');
    ...
    loadQuery.then((snapshot) => {
        ...
        labTicketsAllDataLoaded = loadAllData;
        ...
        console.error('❌ Error en carga inicial de Firebase:', error);
    });
...
function setupLabIncrementalListeners() {
    const handleChildAdded = (snapshot) => {
        const ticket = { ...snapshot.val(), firebaseKey: snapshot.key };
        
        // Solo procesar tickets recientes si no se cargaron todos los datos
        if (!labTicketsAllDataLoaded) {
            const ticketDate = ticket.fechaCreacion ? ticket.fechaCreacion.split('T')[0] : '';
            const cutoffDate = new Date();
            cutoffDate.setDate(cutoffDate.getDate() - labTicketsLoadDateRange);
        ...
        if (!labTickets.some(t => t.firebaseKey === ticket.firebaseKey)) {
            labTickets.push(ticket);
            if (ticket.id >= currentLabTicketId) {
                currentLabTicketId = ticket.id + 1;
            }
            ...
            if (labTicketsUpdateDebounceTimer) {
                clearTimeout(labTicketsUpdateDebounceTimer);
            }
            labTicketsUpdateDebounceTimer = setTimeout(() => {
                const labSection = document.getElementById('verLabSection');
                if (labSection && !labSection.classList.contains('hidden')) {
                    const activeFilterBtn = document.querySelector('.lab-filter-btn.active');
                    const currentStateFilter = activeFilterBtn ? activeFilterBtn.getAttribute('data-filter') : 'todos';
    ...
    const handleChildChanged = (snapshot) => {
        ...
        const index = labTickets.findIndex(t => t.firebaseKey === snapshot.key);
        
        if (index !== -1) {
            labTickets[index] = updatedTicket;
        } else {
            labTickets.push(updatedTicket);
        }
...
function handleLabTicketSubmit(e) {
    ...
    const formData = {
        ...
        empresa: getCurrentEmpresa() // Agregar empresa actual (permite a admins crear en empresa seleccionada)
    };
...
function formatDate(dateString) {
    ...
    // Si la fecha ya está en formato YYYY-MM-DD, parsearla manualmente para evitar problemas de zona horaria
    if (typeof dateString === 'string' && /^\d{4}-\d{2}-\d{2}/.test(dateString)) {
        ...
        if (parts.length >= 3) {
            ...
            const month = parts[1];
            const day = parts[2];
            // Formatear directamente sin usar Date para evitar problemas de zona horaria
...
function updateLabStats() {
    ...
    // Aplicar filtro por departamento según el rol del usuario
    const visibleTickets = filterTicketsByUserRole(ticketsByEmpresa);
