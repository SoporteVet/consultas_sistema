...
function selectConsultorioVoice() {
    ...
    if (consultorioAnnouncementVoice && voices.includes(consultorioAnnouncementVoice)) {
        return consultorioAnnouncementVoice;
    }

    const normalizedVoices = voices.filter(Boolean);

    const lowerMatch = (value, pattern) => (value || '').toLowerCase().includes(pattern.toLowerCase());
    const isFemale = voice => {
        const name = (voice.name || '').toLowerCase();
        const gender = (voice.gender || '').toLowerCase();
        return /(female|woman|mujer|femenina|feminine|femenino|hembra)/i.test(name) || gender === 'female';
    };

    const sabinaVoice = normalizedVoices.find(voice =>
        lowerMatch(voice.name, 'sabina') && voice.lang && voice.lang.toLowerCase().startsWith('es')
    );
    if (sabinaVoice) {
        consultorioAnnouncementVoice = sabinaVoice;
        console.info('[Consultorio] Voz seleccionada (preferida):', consultorioAnnouncementVoice.name, consultorioAnnouncementVoice.lang);
        return consultorioAnnouncementVoice;
    }

    const matchers = [
        voice => voice.lang && voice.lang.toLowerCase().startsWith('es') && lowerMatch(voice.name, 'google') && (isFemale(voice) || lowerMatch(voice.name, 'femen')),
        voice => voice.lang && voice.lang.toLowerCase().startsWith('es-us') && lowerMatch(voice.name, 'google'),
        voice => voice.lang && voice.lang.toLowerCase().startsWith('es-mx') && lowerMatch(voice.name, 'google'),
        voice => voice.lang && voice.lang.toLowerCase().startsWith('es') && lowerMatch(voice.name, 'google'),
        voice => voice.lang && voice.lang.toLowerCase().startsWith('es') && isFemale(voice),
        voice => isFemale(voice),
        voice => voice.lang && voice.lang.toLowerCase().startsWith('es')
    ];

    for (const matcher of matchers) {
        const found = normalizedVoices.find(matcher);
        if (found) {
            consultorioAnnouncementVoice = found;
            break;
        }
    }

    if (!consultorioAnnouncementVoice) {
        consultorioAnnouncementVoice = normalizedVoices[0] || null;
    }

    if (consultorioAnnouncementVoice) {
        console.info('[Consultorio] Voz seleccionada:', consultorioAnnouncementVoice.name, consultorioAnnouncementVoice.lang);
    } else {
        console.warn('[Consultorio] No se encontró voz disponible en speechSynthesis.');
    }

    return consultorioAnnouncementVoice;
}

function getConsultorioNumero(estado) {
    if (!estado) return '';
    const match = estado.toLowerCase().match(/consultorio\s*(\d+)/i) || estado.toLowerCase().match(/consultorio(\d+)/i);
    if (match && match[1]) return match[1];
    return estado.replace(/consultorio/i, '').trim();
}

function getNombrePaciente(ticket) {
    const raw = (ticket.mascota || '').trim();
    if (!raw) return '';
    const parts = raw.split('-').map(part => part.trim()).filter(Boolean);
    return parts.length ? parts[0] : raw;
}

function getApellidoCliente(ticket) {
    const raw = (ticket.nombre || '').trim();
    if (!raw) return '';
...
// Variable global para rastrear la última notificación mostrada
let lastNotificationTime = 0;
let lastNotificationTicketId = null;

function showConsultorioNotificationBox(ticket, consultorioNumero) {
    const userRoleRaw = sessionStorage.getItem('userRole') || '';
    const userRole = userRoleRaw.toLowerCase();
    if (userRole !== 'visitas') return;
    
    const currentFilterBtn = document.querySelector('.filter-btn.active');
    const currentFilter = currentFilterBtn
        ? currentFilterBtn.getAttribute('data-filter')
        : (userRole === 'visitas' ? 'espera' : null);
    if (currentFilter && currentFilter !== 'espera') return;
    
    // PROTECCIÓN: Evitar mostrar la misma notificación múltiples veces en un corto período
    const ticketId = ticket.id || ticket.randomId || ticket.firebaseKey;
    const now = Date.now();
    const timeSinceLastNotification = now - lastNotificationTime;
    
    // Si es el mismo ticket y pasaron menos de 2 segundos, no mostrar de nuevo
    if (lastNotificationTicketId === ticketId && timeSinceLastNotification < 2000) {
        return;
    }
    
    // Actualizar el registro de la última notificación
    lastNotificationTime = now;
    lastNotificationTicketId = ticketId;
    
    const nombrePaciente = getNombrePaciente(ticket) || 'Paciente';
    const apellidoCliente = getApellidoCliente(ticket) || '';
    const nombreClienteCompleto = (ticket.nombre || '').trim();
    
    // Crear estilos si no existen
    if (!document.getElementById('consultorio-notification-styles')) {
        const style = document.createElement('style');
        style.id = 'consultorio-notification-styles';
        style.innerHTML = `
            .consultorio-notification-box {
                position: fixed;
                top: 50%;
                left: 50%;
            ...
            .consultorio-notification-box.show {
                transform: translate(-50%, -50%) scale(1);
                opacity: 1;
            }
            
            .consultorio-notification-box .icon {
                font-size: 48px;
                margin-bottom: 15px;
            ...
            .consultorio-notification-box .title {
                font-size: 24px;
                font-weight: bold;
                margin-bottom: 20px;
                text-transform: uppercase;
            ...
            .consultorio-notification-box .info {
                font-size: 18px;
                margin: 10px 0;
                line-height: 1.6;
            }
            
            .consultorio-notification-box .info strong {
                font-weight: 600;
                display: block;
                margin-top: 15px;
                font-size: 20px;
            ...
            .consultorio-notification-box .consultorio-number {
                font-size: 32px;
                font-weight: bold;
                margin-top: 15px;
    ...
    const existingBox = document.querySelector('.consultorio-notification-box');
    if (existingBox) {
        existingBox.remove();
    }

    console.info('[Consultorio] Mostrando aviso visual:', {
        paciente: nombrePaciente,
        cliente: nombreClienteCompleto || apellidoCliente,
        consultorio: consultorioNumero
    });
    
    // Crear el cuadro de notificación
    const notificationBox = document.createElement('div');
    notificationBox.className = 'consultorio-notification-box';
    notificationBox.innerHTML = `
        <div class="icon">
            <i class="fas fa-user-md"></i>
        </div>
        <div class="title">PASE AL CONSULTORIO</div>
        <div class="info">
            <strong>PACIENTE:</strong> ${nombrePaciente.toUpperCase()}
            ${nombreClienteCompleto ? `<br><strong>CLIENTE:</strong> ${nombreClienteCompleto.toUpperCase()}` : ''}
        </div>
        <div class="consultorio-number">CONSULTORIO ${consultorioNumero}</div>
    `;
...
function announceConsultorio(ticket, nuevoEstado, estadoAnterior) {
    // VERIFICACIÓN: Solo ejecutar para usuarios 'visitas'
    const userRoleRaw = sessionStorage.getItem('userRole') || '';
    const userRole = userRoleRaw.toLowerCase();
    if (userRole !== 'visitas') return;
    ...
    if (!estadoLower.startsWith('consultorio')) return;
    
    const previousLower = (estadoAnterior || '').toLowerCase();
    // MODIFICACIÓN: Permitir desde cualquier estado, no solo desde 'espera'
    // if (previousLower !== 'espera') return;

    const tipoServicio = (ticket.tipoServicio || '').toLowerCase();
    // MODIFICACIÓN: Eliminada la restricción de tipo de servicio - ahora funciona para todos los tipos
    // El anuncio se reproduce para cualquier ticket que cambie a consultorio, sin importar el tipo de servicio

    const consultorioNumero = getConsultorioNumero(nuevoEstado);
    if (!consultorioNumero) return;

    const nombrePaciente = getNombrePaciente(ticket) || 'el paciente';
    const apellidoCliente = getApellidoCliente(ticket);

    // Mostrar cuadro visual para usuarios visitas en filtro espera
    showConsultorioNotificationBox(ticket, consultorioNumero);

    if (!window.speechSynthesis || typeof window.SpeechSynthesisUtterance !== 'function') return;

    const mensaje = `El paciente ${nombrePaciente}${apellidoCliente ? ' ' + apellidoCliente : ''} favor pasar al consultorio ${consultorioNumero}`;
    
    // Función para crear y reproducir el utterance
    const speakMessage = (repeatCount = 0) => {
        const utterance = new SpeechSynthesisUtterance(mensaje);
        utterance.lang = 'es-ES';
        utterance.rate = 0.95;
        utterance.pitch = 1.05;

        const voice = selectConsultorioVoice();
        if (voice) {
            utterance.voice = voice;
        } else {
            // Si la voz no está disponible, esperar a que esté lista
            const onVoicesChanged = () => {
                const refreshedVoice = selectConsultorioVoice();
                if (refreshedVoice) {
                    utterance.voice = refreshedVoice;
                }
                
                // Configurar repetición antes de hablar
                utterance.onend = () => {
                    if (repeatCount < 1) {
                        setTimeout(() => {
                            speakMessage(repeatCount + 1);
                        }, 300);
                    }
                };
        ...
        // Repetir el mensaje 2 veces
        utterance.onend = () => {
            if (repeatCount < 1) {
                // Repetir una vez más (total: 2 veces)
                setTimeout(() => {
                    speakMessage(repeatCount + 1);
                }, 300); // Pequeña pausa entre repeticiones
            }
        };
    ...
    const handleChildChanged = (snapshot) => {
        ...
        if (index !== -1) {
            ...
            if (
                ...
                updatedTicket.estado.toLowerCase().startsWith('consultorio')
            ) {
...
function addTicket() {
    try {
        ...
        const userEmpresa = getUserEmpresa();
        // Obtener la empresa actual (permite a admins crear en empresa seleccionada)
        const currentEmpresa = getCurrentEmpresa();
        // Filtrar tickets de hoy SOLO de la misma empresa para numeración independiente
        const ticketsHoy = tickets.filter(t => t.fechaConsulta === hoy && t.empresa === currentEmpresa);
        ...
        const nuevoTicket = {
            ...
            porCobrar,
            expediente,
            empresa: currentEmpresa, // Agregar empresa actual (permite a admins crear en empresa seleccionada)
...
function renderTickets(filter = 'todos', date = null) {
    ...
    // Filtrar por empresa - obtener la empresa actual del usuario o la seleccionada por admin
    const currentEmpresa = getCurrentEmpresa();
    // Primero filtrar todos los tickets por empresa
    let ticketsByEmpresa = tickets.filter(t => {
    ...
    if (filter === 'todos') {
        ...
        filteredTickets = ticketsByEmpresa.filter(t => t.fechaConsulta === selectedDate && t.estado !== 'terminado' && t.estado !== 'cliente_se_fue');
    } else if (filter === 'espera') {
        // Para tickets de consulta en espera, mostrar todos sin importar la fecha
        filteredTickets = ticketsByEmpresa.filter(ticket => ticket.estado === 'espera');
    } else if (filter === 'consultorio') {
        filteredTickets = ticketsByEmpresa.filter(ticket => 
            (
        ...
        filteredTickets = ticketsByEmpresa.filter(ticket => (ticket.estado === 'terminado' || ticket.estado === 'cliente_se_fue') && ticket.fechaConsulta === selectedDate);
    } else if (filter === 'urgentes') {
        filteredTickets = ticketsByEmpresa.filter(ticket => ticket.urgencia === 'alta' && ticket.fechaConsulta === selectedDate);
    } else if (filter === 'lista' && sessionStorage.getItem('userRole') !== 'visitas') {
        // Filtro lista: igual que 'todos' pero en formato tabla
        filteredTickets = ticketsByEmpresa.filter(t => t.fechaConsulta === selectedDate);
        ...
        filteredTickets = ticketsByEmpresa.filter(ticket => ticket.fechaConsulta === selectedDate);
    }
    ...
    filteredTickets.forEach((ticket, index) => {
        ...
        if (!hasPermission('canViewFullTicket')) {
            ...
            // Badge de empresa para administradores
            let empresaBadge = '';
            if (hasPermission('canViewStats') && ticket.empresa) {
                const empresaName = ticket.empresa === 'veterinaria_smp' ? 'Vet. SMP' : 'Inst. SMP';
                const empresaColor = ticket.empresa === 'veterinaria_smp' ? '#4CAF50' : '#9C27B0';
                empresaBadge = `<div style="background: ${empresaColor}; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; margin-top: 5px; display: inline-block;">
                    <i class="fas fa-building" style="font-size: 10px;"></i> ${empresaName}
                </div>`;
            }
...
function saveEditedTicket(ticket) {
    ...
    const previousEstado = typeof oldTicket.estado === 'string' ? oldTicket.estado : null;
...
// Función para inicializar el selector de empresa para administradores
function initEmpresaSelector() {
    const empresaSelectorContainer = document.getElementById('empresaSelectorContainer');
    const empresaSelector = document.getElementById('empresaSelector');
    const currentEmpresaText = document.getElementById('currentEmpresaText');
    const salaEsperaEmpresaName = document.getElementById('salaEsperaEmpresaName');
    
    if (!empresaSelectorContainer || !empresaSelector) {
        return;
    }
    
    // Mostrar selector solo para administradores
    const userRole = sessionStorage.getItem('userRole');
    if (hasPermission('canViewStats')) {
        ...
        const currentEmpresa = getCurrentEmpresa();
        empresaSelector.value = currentEmpresa;
        
        // Actualizar texto de empresa actual
        updateEmpresaDisplay(currentEmpresa);
        
        // Manejar cambio de empresa
        empresaSelector.addEventListener('change', function() {
            const newEmpresa = this.value;
            setCurrentEmpresa(newEmpresa);
            updateEmpresaDisplay(newEmpresa);
            ...
            // Re-renderizar laboratorio si está activo
            ...
            if (labSection && !labSection.classList.contains('hidden')) {
                ...
                if (typeof renderLabTickets === 'function') {
                    renderLabTickets(currentLabFilter, currentMedicoFilter);
                }
                // Actualizar estadísticas de laboratorio con el nuevo filtro de empresa
                if (typeof updateLabStats === 'function') {
                    updateLabStats();
                }
            }
            
            // Re-renderizar quirófano si está activo
            ...
            if (quirofanoSection && !quirofanoSection.classList.contains('hidden')) {
                if (typeof renderQuirofanoTicketsWithDateFilter === 'function') {
                    const quirofanoFilter = document.querySelector('.quirofano-filter-btn.active');
                    const currentQuirofanoFilter = quirofanoFilter ? quirofanoFilter.getAttribute('data-filter') : 'todos';
                    const quirofanoDateFilter = document.getElementById('quirofanoDateFilter');
                    const currentQuirofanoDate = quirofanoDateFilter ? quirofanoDateFilter.value : '';
                    renderQuirofanoTicketsWithDateFilter(currentQuirofanoFilter, '', currentQuirofanoDate);
                }
            }
            
            // Notificar cambio
            showNotification(`Cambiado a ${getEmpresaDisplayName(newEmpresa)}`, 'success');
        });
    } else {
        empresaSelectorContainer.style.display = 'none';
        // Para usuarios no admin, solo actualizar el nombre en el título
        const userEmpresa = getUserEmpresa();
        updateEmpresaDisplay(userEmpresa);
    }
}

// Función para actualizar el display de empresa en la UI
function updateEmpresaDisplay(empresaCode) {
    const currentEmpresaText = document.getElementById('currentEmpresaText');
    const salaEsperaEmpresaName = document.getElementById('salaEsperaEmpresaName');
    const displayName = getEmpresaDisplayName(empresaCode);
    const shortName = empresaCode === 'veterinaria_smp' ? 'Veterinaria SMP' : 'Instituto SMP';
    
    if (currentEmpresaText) {
        currentEmpresaText.textContent = displayName;
    }
    if (salaEsperaEmpresaName) {
        salaEsperaEmpresaName.textContent = shortName;
    }
}
  ...
  document.querySelectorAll('.filter-btn:not(#labFilterBtn)').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      if (labResultsContainer) {
        labResultsContainer.classList.add('hidden');
      }
      if (ticketContainer) {
...
// Importar lab.js
// Comentado porque laboratorio.js ya se carga en index.html
// const scriptLab = document.createElement('script');
// scriptLab.src = 'js/laboratorio.js';
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  // scriptQuirofano.src = 'js/quirofano-module.js';
