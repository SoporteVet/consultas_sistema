...
function updateTotal() {
    ...
    if (cedulaInput) {
        cedulaInput.addEventListener('change', function() {
            ...
            // Buscar ticket por cédula (ajusta según tu estructura de datos)
            let ticket = null;
            if (window.tickets && Array.isArray(window.tickets)) {
                ticket = window.tickets.find(t => t.cedula === cedula);
            }

            if (ticket) {
                // Llenar datos del propietario
                document.querySelector('input[name="propietario"]').value = ticket.nombre || '';
                document.querySelector('input[name="telefono"]').value = ticket.telefono || '';
                document.querySelector('input[name="correo"]').value = ticket.correo || '';

                // Llenar datos del paciente
                document.querySelector('input[name="nombre-paciente"]').value = ticket.mascota || '';
                document.querySelector('input[name="raza"]').value = ticket.raza || '';
                document.getElementById('especieSelector').value = ticket.especie || '';
                document.querySelector('select[name="sexo"]').value = ticket.sexo || '';
                document.querySelector('input[name="edad"]').value = ticket.edad || '';
                document.querySelector('select[name="unidad-edad"]').value = ticket.unidadEdad || '';
                document.querySelector('input[name="peso"]').value = ticket.peso || '';
            }
        });
    }
});
...
if (filasVisibles.length === 0) {
    ...
    if (filaTotal) filaTotal.style.display = 'none';
        ...
        // Subir el PDF al backend para Google Drive
        await subirPDFaDrive(pdfBlob, fileName);
...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwyH0emLkE6uLYzA9GQbG19WcPo2qCcxB5h2MzzhOgDNkb_3wwh7h_9NaiZ5VVBJetdNA/exec'; // Pega aquí tu URL real

async function subirPDFaDrive(pdfBlob, fileName) {
    const formData = new FormData();
    // Forzar el tipo MIME a PDF
    ...
    try {
        ...
        alert('PDF enviado a Google Drive correctamente.');
        // Puedes mostrar el enlace al archivo en Drive si el Apps Script lo retorna
        // alert('Archivo en Drive: ' + result.url);
    } catch (err) {
        alert('No se pudo subir el PDF a Google Drive: ' + err.message);
    }
}
